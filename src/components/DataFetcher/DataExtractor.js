// src/components/layout/DataExtractor.js
...
const DataExtractor = ({ book }) => {
  ...
  const coverImage = volumeInfo.imageLinks?.thumbnail || 'https://via.placeholder.com/128x194';
  const price = saleInfo.retailPrice ? Math.floor(saleInfo.retailPrice.amount) : 'Not Available';
  ...
  function getAuthors(volumeInfo) {
    ...
    return authorsArray.length > 2 ? authorsArray.slice(0, 1) + ', ...' : authorsArray[0];
  }
